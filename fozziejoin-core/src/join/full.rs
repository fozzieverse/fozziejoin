...
impl FuzzyJoin {
    pub fn full(
        ...
        let rhs_complement: Vec<u32> = (0..=(rhs_len - 1) as u32)
            ...
            .collect();
        let right_complement2 = UInt32Chunked::from_slice("right_idx".into(), &rhs_complement);
        let mut right_unmatched = right
            .take(&right_complement2)
            .map_err(|e| anyhow!("Failed to take right rows: {e}"))?;
        for col in right_unmatched.get_column_names_owned() {
            // If shared, add suffix to right hand side
            if left_names.contains(col.as_str()) {
                right_unmatched
                    .rename(col.as_str(), format!("{}{}", col, suffix).into())
        ...
        let right_unmatched_lz = right_unmatched.lazy();

        let out: LazyFrame = concat([joined, right_unmatched_lz], args)?;
        let out: DataFrame = out.collect()?;
