use crate::stringdist::string_dist_method::StringDistance;
use anyhow::Result;
use itertools::iproduct;
use rayon::prelude::*;
use rustc_hash::FxHashMap;

pub struct LCS;

impl LCS {
    fn compute(&self, s1: &str, s2: &str) -> usize {
        let m = s1.len();
        let n = s2.len();
        let mut dp = vec![vec![0; n + 1]; m + 1];

        for (i, c1) in s1.chars().enumerate() {
            for (j, c2) in s2.chars().enumerate() {
                if c1 == c2 {
                    dp[i + 1][j + 1] = dp[i][j] + 1;
                } else {
                    dp[i + 1][j + 1] = dp[i + 1][j].max(dp[i][j + 1]);
                }
            }
        }

        (m + n) - 2 * dp[m][n]
    }

    fn compare_one_to_many(
        &self,
        k1: &str,
        v1: &Vec<usize>,
        length_map: &FxHashMap<usize, Vec<&str>>,
        idx_map: &FxHashMap<&str, Vec<usize>>,
        max_distance: &f64,
    ) -> Option<Vec<(usize, usize, f64)>> {
        // Get range of lengths within max distance of current
        let k1_len = k1.len();
        let start_len = k1_len.saturating_sub(*max_distance as usize);
        let end_len = k1_len.saturating_add(*max_distance as usize + 1);

        // Start a list to collect results
        let mut idxs: Vec<(usize, usize, f64)> = Vec::new();

        // Begin making string comparisons
        for i in start_len..end_len {
            if let Some(lookup) = length_map.get(&i) {
                lookup.iter().for_each(|k2| {
                    // No need to run distance functions if exactly the same
                    if &k1 == k2 {
                        let v2 = idx_map.get(k2).unwrap();
                        iproduct!(v1, v2).for_each(|(v1, v2)| {
                            idxs.push((*v1, *v2, 0.));
                        });
                        return;
                    }

                    // Run distance calculation
                    let dist = self.compute(&k1, &k2) as f64;

                    // Check vs. threshold
                    if dist <= *max_distance {
                        let v2 = idx_map.get(k2).unwrap();
                        iproduct!(v1, v2).for_each(|(v1, v2)| {
                            idxs.push((*v1, *v2, dist));
                        });
                        return;
                    }
                });
            }
        }

        // Return all matches, if any
        if idxs.is_empty() {
            return None;
        } else {
            return Some(idxs);
        }
    }
}

impl StringDistance for LCS {
    fn compare_pairs(
        &self,
        left: &Vec<Option<String>>,
        right: &Vec<Option<String>>,
        max_distance: &f64,
        _q: &Option<usize>,
        _prefix_weight: Option<f64>,
        _max_prefix: Option<usize>,
        pool: &rayon::ThreadPool,
    ) -> Result<(Vec<usize>, Vec<f64>)> {
        let (keep, dists): (Vec<usize>, Vec<f64>) = pool.install(|| {
            left.par_iter()
                .zip(right)
                .enumerate()
                .filter_map(|(i, (l, r))| {
                    let l = match l {
                        Some(x) => x,
                        None => return None,
                    };
                    let r = match r {
                        Some(x) => x,
                        None => return None,
                    };

                    let dist = self.compute(l, r) as f64;
                    if dist <= *max_distance {
                        Some((i, dist))
                    } else {
                        None
                    }
                })
                .unzip()
        });
        Ok((keep, dists))
    }

    fn fuzzy_indices(
        &self,
        left: &Vec<Option<String>>,
        right: &Vec<Option<String>>,
        max_distance: &f64,
        _q: &Option<usize>,
        _prefix_weight: Option<f64>,
        _max_prefix: Option<usize>,
        pool: &rayon::ThreadPool,
    ) -> anyhow::Result<Vec<(usize, usize, f64)>> {
        let mut map1: FxHashMap<&str, Vec<usize>> = FxHashMap::default();
        left.iter().enumerate().for_each(|(index, val)| match val {
            Some(x) => map1.entry(x).or_default().push(index),
            None => (),
        });

        let mut map2: FxHashMap<&str, Vec<usize>> = FxHashMap::default();
        right.iter().enumerate().for_each(|(index, val)| match val {
            Some(x) => map2.entry(x).or_default().push(index),
            None => (),
        });

        let mut length_map: FxHashMap<usize, Vec<&str>> = FxHashMap::default();
        for key in map2.keys() {
            let key_len = key.len();
            length_map.entry(key_len).or_default().push(key);
        }

        let idxs: Vec<(usize, usize, f64)> = pool.install(|| {
            map1.par_iter()
                .filter_map(|(k1, v1)| {
                    self.compare_one_to_many(k1, v1, &length_map, &map2, &max_distance)
                })
                .flatten()
                .collect()
        });

        Ok(idxs)
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_lcs_empty_strings() {
        let lcs = LCS;
        assert_eq!(lcs.compute("", ""), 0);
    }

    #[test]
    fn test_lcs_one_empty_string() {
        let lcs = LCS;
        assert_eq!(lcs.compute("abc", ""), 3);
        assert_eq!(lcs.compute("", "abc"), 3);
    }

    #[test]
    fn test_lcs_identical_strings() {
        let lcs = LCS;
        assert_eq!(lcs.compute("abc", "abc"), 0);
    }

    #[test]
    fn test_lcs_partial_match() {
        let lcs = LCS;
        assert_eq!(lcs.compute("abc", "ab"), 1);
        assert_eq!(lcs.compute("abcdef", "ace"), 3);
    }

    #[test]
    fn test_lcs_no_match() {
        let lcs = LCS;
        assert_eq!(lcs.compute("abc", "def"), 6);
    }

    #[test]
    fn test_compare_pairs() {
        let lcs = LCS;
        let left = vec![Some("apple".to_string()), Some("banana".to_string())];
        let right = vec![Some("apple".to_string()), Some("grape".to_string())];
        let max_distance = &1.0; // Allow only exact matches

        // Prepare a thread pool
        let pool = crate::utils::get_pool(None).unwrap();

        let (keep, dists) = lcs
            .compare_pairs(&left, &right, max_distance, &None, None, None, &pool)
            .unwrap();
        assert_eq!(keep, vec![0]);
        assert_eq!(dists, vec![0.0]);
    }

    #[test]
    fn test_fuzzy_indices() {
        let lcs = LCS;
        let left = vec![Some("apple".to_string()), Some("banana".to_string())];
        let right = vec![Some("aplpe".to_string()), Some("grape".to_string())];
        let max_distance = &1.0; // Allow slight variations

        // Prepare a thread pool
        let pool = crate::utils::get_pool(None).unwrap();
        let idxs = lcs
            .fuzzy_indices(&left, &right, max_distance, &None, None, None, &pool)
            .unwrap();
        assert_eq!(idxs, vec!());
    }
}
