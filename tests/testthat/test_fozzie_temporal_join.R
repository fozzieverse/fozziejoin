test_that("temporal inner join matches within seconds", {
  df1 <- data.frame(time = as.POSIXct(c("2023-01-01 12:00:00", "2023-01-01 13:00:00"), tz='UTC'))
  df2 <- data.frame(time = as.POSIXct(c("2023-01-01 12:00:05", "2023-01-01 14:00:00"), tz='UTC'))

  result <- fozzie_temporal_inner_join(df1, df2, by = c("time"), max_distance = 10, unit = "seconds")
  expect_equal(nrow(result), 1)
  expect_equal(result$time.x, as.POSIXct("2023-01-01 12:00:00", tz='UTC'))
  expect_equal(result$time.y, as.POSIXct("2023-01-01 12:00:05", tz='UTC'))
})

test_that("temporal inner join matches within seconds on POSIXlt", {
  df1 <- data.frame(time = as.POSIXlt(c("2023-01-01 12:00:00", "2023-01-01 13:00:00"), tz='UTC'))
  df2 <- data.frame(time = as.POSIXlt(c("2023-01-01 12:00:05", "2023-01-01 14:00:00"), tz='UTC'))
